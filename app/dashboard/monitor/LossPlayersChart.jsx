    ...
    <div className={styles.chartWrap}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 48 }}>
          ...
          <XAxis dataKey="name" interval={0} angle={-35} textAnchor="end" height={60} tick={{ fontSize: 12, fill: "#fff" }} />
          <YAxis tick={{ fontSize: 12, fill: "#fff" }} />
          <Tooltip />
          <Bar dataKey="loss" fill={BAR_COLOR} radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>

      <div className={styles.legend}>(Barra = puntos perdidos)</div>
    </div>
  );
}
