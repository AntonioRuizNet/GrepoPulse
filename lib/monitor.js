...
export async function getMonitorData(worldId, limit = 10) {
  ...
  const conquersTodayCount = await prisma.conquer.count({
    where: { worldId, time: { gte: todayEpoch } },
  });

  // 1) CONQUISTAS DEL DÍA (para agrupar por mares)
  // Cargamos suficientes filas para poder obtener 10 por mar.
  ...
  // Agrupar en tablas por mar (10 registros por mar)
  const bySea = new Map();
  for (const r of conquersRows) {
    if (r.sea == null) continue;
    if (!bySea.has(r.sea)) bySea.set(r.sea, []);
    const arr = bySea.get(r.sea);
    if (arr.length < 10) arr.push(r);
  }
  const conquersBySea = Array.from(bySea.entries())
    .sort((a, b) => Number(a[0]) - Number(b[0]))
    .map(([sea, rows]) => ({ sea, rows }));
  ...
  // KPIs para tarjetas
  const kpis = {
    conquersToday: conquersTodayCount,
